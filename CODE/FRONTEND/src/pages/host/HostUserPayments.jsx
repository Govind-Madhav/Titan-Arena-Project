...
const HostUserPayments = () => {
  ...
  const computeTotalEarnings = (txList) =>
    txList
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (txRes.data.success) {
          ...
          setWalletStats((prev) => ({
            ...prev,
            totalEarnings: computeTotalEarnings(txList)
          }));
        }
      } catch (error) {
        console.error(error);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-12">
            ...
            <SpotlightCard className="bg-titan-bg-card border-white/10 p-6 flex items-center gap-4">
              ...
              <div>
                <p className="text-white/40 text-sm">Total Earnings (Completed Credits)</p>
                <h3 className="text-2xl font-bold text-white">₹{(walletStats.totalEarnings / 100).toLocaleString()}</h3>
              </div>
            </SpotlightCard>
          </div>
